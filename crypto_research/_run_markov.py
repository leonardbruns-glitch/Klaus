"""Build BTC panels once, run the four Markov strategies through fresh engines.

Usage: python3 -m crypto_research._run_markov START END
"""
...
STRATS = ["markov_regime_gate", "markov_mid_revert",
          "markov_flow_follow", "markov_mdp_stop"]
...
for name in STRATS:
    ...
    res = engine.run()
    o = res["metrics_overall"]
    print(f"\n=== {name} ===", flush=True)
    print(f"  n={o['n_trades']}  WR={o['win_rate']:.3f}  PF={o['profit_factor']:.3f}  "
          f"Sortino={o['sortino']:.3f}  PnL={o['total_net_pnl']:.2f}  "
